...
**C. 有测试、口径不够**
16. 层 1 在**真实病历**上的读数仍为零。2026-09-22 已把题面从模板短句换成**期刊体八节病历**(章节骨架取自 84 篇开放获取病例报告的统计,内容由金标填充,haenv P7 段),这让章节切分、位置对位、噪声弃权三件事第一次可测(并当场抓到 split_sections 的缺陷);但文体仍是渲染的,不是真实就诊记录,D9 金标(30 份人工标注)未做——那仍是唯一一条合成题证明不了的验收。**语料要分两半**:抽骨架的那一半不能同时做金标,否则真实语料读数虚高。
