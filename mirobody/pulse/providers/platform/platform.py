...
class ProviderPlatform(Platform):
    ...
    async def sync_user_devices(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"provider platform sync_user_devices: found {len(connected_providers)} connected providers for user {user_id}"
            )
            ...
            logger.error(f"Error syncing theta devices for user {user_id}: {str(e)}")
            return False
    ...
    async def get_webhooks(
        ...
        For provider platform, 'provider' parameter is REQUIRED to specify which provider's data to query.
        This method delegates to the provider's get_raw_data_records method.
        
        Args:
            page: Page number (starting from 1)
            page_size: Number of records per page
            provider: Provider slug (REQUIRED for this platform, e.g., 'theta_garmin')
            event_type: Not used by this platform (kept for interface compatibility)
            user_id: Optional filter by user ID
            status: Not used by this platform (kept for interface compatibility)
        ...
        """
        if not provider:
            raise ValueError("Provider parameter is required for provider platform")
        
        logger.info(f"Getting webhooks for provider {provider}: page={page}, page_size={page_size}, user_id={user_id}")
        
        # Get provider instance
        provider_instance = self.get_provider(provider)
        if not provider_instance:
            raise ValueError(f"Provider '{provider}' not found in provider platform")
        ...
        for record in raw_data_result.get("records", []):
            webhook = {
                ...
                "team_id": "",  # This platform doesn't have team_id
                "msg_id": record.get("msg_id", ""),
                "req_id": "",  # This platform doesn't have req_id
                "status": 1,  # Assume success (1) for existing records
                "app_user_id": record.get("theta_user_id"),
                "is_event_type_implemented": True,  # providers are all implemented
            }
    ...
    async def check_format(self, webhook_id: int, provider: str | None = None) -> dict[str, Any]:
        ...
        For provider platform, 'provider' parameter is REQUIRED to specify which provider's data to check.
        ...
        logger.info(f"Checking format for provider {provider}, webhook_id: {webhook_id}")
        ...
        try:
            ...
            formatted_pulse_data = await provider_instance.format_data(fmt_input)
