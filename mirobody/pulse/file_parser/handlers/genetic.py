...
class GeneticHandler(BaseFileHandler):
    ...
    # Marker every WeGene export starts with: the sole genetic-file signal.
    GENETIC_MARKER = "# This data file generated by WeGene at"

    @staticmethod
    def is_genetic_content(head: Any, content_type: str | None) -> bool:
        """Genetic-file check on an already-read header (bytes or str).

        Shared by ``is_genetic_file`` (UploadFile path) and the chat upload path
        (raw bytes), so both classify WeGene files identically. A genetic file is
        ``text/plain`` whose header carries the WeGene marker.
        """
        if content_type and content_type != "text/plain":
            return False
        if isinstance(head, (bytes, bytearray)):
            head = head.decode("utf-8", errors="ignore")
        return GeneticHandler.GENETIC_MARKER in (head or "")
    ...
    async def is_genetic_file(file: UploadFile) -> bool:
        ...
        try:
            ...
            return GeneticHandler.is_genetic_content(first_line, file.content_type)
        except Exception:
            return False
