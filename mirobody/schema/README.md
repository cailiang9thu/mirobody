...
The reorganization was checked by applying each chain to a scratch database
and diffing the catalogue: the old chain against the new one, the new one
applied twice, and the old chain followed by the new one. Replaying is
idempotent, so any of the three may be run again on a live database.
