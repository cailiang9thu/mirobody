...
async def standardize(body: StandardizeRequest, user_id: str = Depends(verify_token)):
    ...
    `measured_at` is the collection date printed on the report, and a stored
    reading is written under it. It was hardcoded `None` until 2026-09-20,
    which meant `_parse_time` used the wall clock: an outside deployment
    report uploaded a May report and an August one, and the August value came
        ...
        data.append(
            {
                ...
                "measured_at": reading.collected or None,
            }
        )
